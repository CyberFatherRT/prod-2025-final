...
@Composable
fun VerificationsScreen(navController: NavController, vm: VerificationsScreenModel = hiltViewModel()) {
            ...
            Column(Modifier.padding(innerPadding)) {
                Row {
                    SubcomposeAsyncImage(
                        model = verifications[0].user.avatarUrl,
                    ...
                    Column {
                        Text(
                            "${verifications[0].user.name} ${verifications[0].user.surname}",
                            style = MaterialTheme.typography.titleLarge
                        )
                        Text(verifications[0].user.email)
                    }
                }
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text(
                        when (verifications[0].user.role) {
                ...
                if (verifications[0].user.role == UserRole.Guest) {
...
private fun ProfileScreenPreview1() {
    ...
    CompositionLocalProvider(LocalAsyncImagePreviewHandler provides previewHandler) {
        VerificationsScreen(rememberNavController())
    }
}